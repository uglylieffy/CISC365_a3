...
def buildTriangles( slice0, slice1 ):
    ...
    # brute force to find the cloest pair of vertices
    closest_vert0 = slice0.verts[0]
    closest_vert1 = slice1.verts[0]
    min_distance = length(subtract(closest_vert0.coords, closest_vert1.coords)) # find the distance between the first set of slice
    for vert0 in slice0.verts:  
        for vert1 in slice1.verts: 
            # calculate distance b/w every single slice, and compare with each other to find the lowest distance
            distance = length(subtract(vert0.coords, vert1.coords))
            if min_distance > distance: # if calculated distance is smaller, replace it
                min_distance = distance
                closest_vert0 = vert0   # record closest vertex 1   
                closest_vert1 = vert1   # record cooresponding vertex 2 
                #print("min_distance")
                # print(min_distance)
                # print("closest_vert0")
                # print(closest_vert0)
                # print("closest_vert1")
                # print(closest_vert1)
    # print("0: ", slice0.verts)
    # print("1: ", slice1.verts)
    ...
    #
    # [1 mark]
    n = len(slice1.verts)   # n is amount of vertexs in slice1  
    m = len(slice0.verts)   # m is amount of vertexs in slice2

    #cyclic permutation by appending vertices 
    #print(slice0)
    #print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
    index = 0   # set flag
    while 1:
        slice0.verts.append(slice0.verts[index])    # append copies of slice0 onto slice0
        index += 1  # loop thru
        if index == m:  # stop when reach end
            break

    # cut from index - 1, slice0 is original val + last bit of it
    slice0.verts = slice0.verts[index-1:]   
    #for vert0 in slice0.verts:
    #    print("+++", vert0)
    #print(slice0.verts)

    #print(slice0)
    # so same thing to slice1 as well
    index = 0
    while 1:
        slice1.verts.append(slice1.verts[index])
        index += 1
        if index == n:
            break
    # cut for slice1 as well
    slice1.verts = slice1.verts[index-1:]
    #for vert1 in slice1.verts:
    #    print(vert1)
    ...
    # cols dimension is the number of vertices in slice0, rows dimension is the number of vertices in slice1
    minArea = [[-1]*len(slice0.verts) for i in range(0, len(slice1.verts))]
    #print(minArea)
    minDir = [[0]*len(slice0.verts) for i in range(0, len(slice1.verts))]
    #print(minDir)
    ...
    # [2 marks]
    # [YOUR CODE HERE]
    # can only check previous column
    for j in range(1, m):
        minArea[0][j] = triangleArea(slice0.verts[j-1].coords, slice1.verts[0].coords, slice0.verts[j].coords) + minArea[0][j-1]
        minDir[0][j] = Dir.PREV_COL
        #break
    #print(minArea)
    #print(minDir)
    ...
    ## [YOUR CODE HERE]
    # can only check previous row
    for i in range(1, n):
        minArea[i][0] = triangleArea(slice1.verts[i-1].coords, slice0.verts[0].coords, slice1.verts[i].coords) + minArea[i-1][0]
    ...
    for i in range(1, n):
        for j in range(1, m):
            #print(triangleArea(slice1.verts[i].coords, slice0.verts[j-1].coords, slice0.verts[j].coords))
            #print(triangleArea(slice1.verts[i].coords, slice1.verts[i-1].coords, slice0.verts[j].coords))
            # a is future tri area
            a = minArea[i][j-1] + triangleArea(slice1.verts[i].coords, slice0.verts[j-1].coords, slice0.verts[j].coords)
            # b is previour tri area 
            b = minArea[i-1][j] + triangleArea(slice1.verts[i].coords, slice1.verts[i-1].coords, slice0.verts[j].coords)
            if a>b: # if future tri is greater than previous tri
                minArea[i][j] = b   # keep minarea as the previour tri vertexs
                minDir[i][j] = Dir.PREV_ROW
            else:
                minArea[i][j] = a   # else, other way around
    ...
    # [0 marks]
    row_format ="{:>8}" * (m + 1)
    print(row_format.format("", *list(range(m))))
    for i in range(0, n):
        row = []
        for j in range(0, m):
            if i == 0 and j == 0:
                row.append('%d %s' % (minArea[i][j], '.'))
    ...
    while r!=0 or c!=0:
        ...
        if minDir[r][c] == Dir.PREV_ROW:
            triangle = Triangle([slice1.verts[r], slice0.verts[c], slice1.verts[r-1]])
            r = r-1
        else:
            triangle = Triangle([slice1.verts[r], slice0.verts[c], slice0.verts[c-1]])
            c = c-1
